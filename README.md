...
Large Language Models are stateless by default. Each request is processed independently, which means the model does not retain information from previous conversations.

This makes it difficult to build assistants that maintain long-term context.
...
Users can remove stored memories directly from the chat interface using:
```
forget: <memory>
```
...
```bash
git clone https://github.com/your-username/memoria
cd memoria
```
